...
def test_plugin_timestamp_type() -> None:
    ...
    assert type.read_cell("26/04/2022") is None
    assert type.read_cell("2022-04-26T22:00") is None  # No Timezone

    # Read Valid Cells
    assert type.read_cell("2022-04-26")  # Date
    assert type.read_cell("2022-04-26T22:00+08:00")  # Date Time with Timezone
...
def test_plugin_parse_timestamp() -> None:
    ...
    with pytest.raises(ValueError):
        utils.plugin.parse_timestamp("26/04/2022")
    with pytest.raises(ValueError):
        utils.plugin.parse_timestamp("2022-04-26T22:00")

    # Parse Valid Timestamps
    assert utils.plugin.parse_timestamp("2022-04-26")
    assert utils.plugin.parse_timestamp("2022-04-26T22:00+08:00")

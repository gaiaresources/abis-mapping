...
def test_timestamp_type() -> None:
    ...
    assert field.read_cell(models.temporal.Datetime.now())[0]

    # Write Cell
    assert field.write_cell(models.temporal.Datetime.now())[0]
...
def test_timestamp_type_year_month() -> None:
    ...
    # Read valid cells
    assert field.read_cell("2022-04")[0] == models.temporal.YearMonth(year=2022, month=4)  # Year month
    assert field.read_cell("04/2022")[0] == models.temporal.YearMonth(year=2022, month=4)  # Year month
    assert field.read_cell("4/2022")[0] == models.temporal.YearMonth(year=2022, month=4)  # Year month
    assert field.read_cell("04/0022")[0] == models.temporal.YearMonth(year=22, month=4)  # Year month
    assert field.read_cell(models.temporal.YearMonth(2022, 4))[0] == models.temporal.YearMonth(
        year=2022, month=4
    )  # Year month

    # Write cell
    assert field.write_cell(models.temporal.YearMonth(year=2022, month=4))[0] == "2022-04"


def test_ym_equality() -> None:
    """Tests the __eq__ implementation for YearMonth"""
    assert models.temporal.YearMonth(2022, 4) == models.temporal.YearMonth(2022, 4)


def test_year_equality() -> None:
    """Tests the __eq__ implementation for Year"""
    assert models.temporal.Year(2022) == models.temporal.Year(2022)
...
def test_timestamp_type_year() -> None:
    ...
    # Read valid cells
    assert field.read_cell("2022")[0] == models.temporal.Year(2022)
    assert field.read_cell(models.temporal.Year(2022))[0] == models.temporal.Year(2022)
    assert field.read_cell("0022")[0] == models.temporal.Year(22)

    # Write cell
    assert field.write_cell(models.temporal.Year(2022))[0] == "2022"
    assert field.write_cell(models.temporal.Datetime.now())[0]
